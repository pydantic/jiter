...
mod test {
    ...
    #[test]
    fn get_object() {
        let v: JsonValue<'static> = JsonValue::Object(Arc::new(LazyIndexMap::from(vec![
    ...
    #[test]
    fn get_header_array() {
        let v: JsonValue<'static> = JsonValue::Array(Arc::new(smallvec![JsonValue::Null, JsonValue::Bool(true),]));
        let bytes = encode_from_json(&v).unwrap();
    ...
    fn get_het_array() {
        let v: JsonValue<'static> =
            JsonValue::Array(Arc::new(
                smallvec![JsonValue::Int(42), JsonValue::Str("foobar".into()),],
            ));
        let bytes = encode_from_json(&v).unwrap();
    ...
    #[test]
    fn get_u8_array() {
        let v: JsonValue<'static> = JsonValue::Array(Arc::new(smallvec![JsonValue::Int(42), JsonValue::Int(255),]));
        let bytes = encode_from_json(&v).unwrap();
    ...
    fn get_i64_array() {
        let v: JsonValue<'static> =
            JsonValue::Array(Arc::new(smallvec![JsonValue::Int(42), JsonValue::Int(i64::MAX),]));
        let bytes = encode_from_json(&v).unwrap();
